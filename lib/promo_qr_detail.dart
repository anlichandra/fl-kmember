...
import 'package:kasimura_member2/promo_qr_model.dart';
import 'package:kasimura_member2/Constants.dart';

class PromoQRDetailPage extends StatefulWidget{
  PromoQRDetailPage({Key key, this.value});// : super(key: key);

  final String value;

  @override
  _PromoQRDetailPageState createState() => _PromoQRDetailPageState();
}

class _PromoQRDetailPageState extends State<PromoQRDetailPage>{
  String title, subtitle;
  MaterialAccentColor titleBGColor;//, titleFGColor;

  PromoQRModel promoDetail = new PromoQRModel();
  ...
  Widget build(BuildContext context){
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text(promoDetail.judul != null ? promoDetail.judul : '',
          style: TextStyle(
  ...
  void _loadPromoDetail() async{
    ...
    promoDetail = new PromoQRModel();
    //print(promoDetail.nama);

    try{
      response = await post(
        Constants.server + '/api/load_promo_qr/' + widget.value,
    ...
    if(data['success']==1){
      promoDetail = PromoQRModel.fromJson(data['promo'][0]);
